...
enum TradingPeriod(val begin: LocalTime, val end: LocalTime):
  ...
  private def encompasses(time: LocalTime) = time.isNotBefore(begin) && time.isNotAfter(end)

object TradingPeriod:
  given Ordering[TradingPeriod] = (x, y) => x.ordinal.compareTo(y.ordinal)

  def of(time: LocalTime): Option[TradingPeriod] = values.find(_.encompasses(time))
